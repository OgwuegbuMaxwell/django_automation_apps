...
class CompressImageAdmin(admin.ModelAdmin):
    ...
    def org_img_size(self, obj):
        return format_html(f'{obj.original_img.size / (1021*1024):.2f} MB')
    
    def comp_img_size(self, obj):
        size_in_mb = obj.compressed_img.size / (1021*1024)
        if size_in_mb >= 1:
    ...
    list_display = ('user','quality', 'thumbnail', 'org_img_size', 'comp_img_size',  'compressed_at')
